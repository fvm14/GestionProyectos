  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (usuario.rol !== 'disenador') {
        setMensaje('No tienes permiso para acceder como diseñador');
        return;
      }

      localStorage.setItem('token', token);
      localStorage.setItem('usuario', JSON.stringify(usuario));
      navigate('/redirigir'); // ← ruta general para redirigir según rol
    } catch (error) {
      ...
      <Container maxWidth="sm">
        <Paper elevation={4} sx={{ padding: 5, borderRadius: 4 }}>
          <Box sx={{ textAlign: 'center', mb: 3 }}>
            <Avatar sx={{ bgcolor: '#7b1fa2', width: 64, height: 64, margin: '0 auto' }}>
              <DesignServicesIcon fontSize="large" />
            </Avatar>
            <Typography variant="h5" sx={{ mt: 2, fontWeight: 600 }}>
              Ingreso para Diseñadores
            </Typography>
          </Box>
